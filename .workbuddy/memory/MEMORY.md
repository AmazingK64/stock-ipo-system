...
### 评分体系
- 满分100分，8个维度: 行业热度(35)+保荐人(20)+投资者(16)+商业模式(10)+估值(10)+绿鞋(5)+AH折价(2)+盈利(2)
- 行业热度: AI/半导体等风口35分，传统行业8分，其他12分
- 保荐人: 中金/摩根等第一梯队20分，第二梯队16分，第三梯队10分，其他6分
- 投资者: 基石+知名机构(≥3家)16分，基石+无知名9分，无基石0分
- 商业模式: `businessModel`(excellent/good/fair/poor) + `moatLevel`(wide/moderate/narrow/none)，满分10（含小红书讨论加成）
...
- **小红书数据集成**: 网络搜索时自动调用小红书API获取相关讨论，正面信号可增强商业模式评分
...
### 详细打分原因
- **需求**: 在评分详情弹窗中添加详细的打分原因，说明为什么获得相应分数
- **实现**:
  - 修改 `ipoScoring.ts` 服务，为每个评分项添加 `reason` 字段
  - 所有评分维度都包含详细打分原因:
    - 行业热度: 根据不同行业梯队详细说明
    - 保荐人: 根据保荐人梯队详细说明
    - 投资者背景: 根据基石投资者和知名机构数量详细说明
    - 其他维度: 商业模式、估值合理性、绿鞋机制、AH折价、盈利能力
...
- **新等级标准**:
  ...
  - 55-59分: B级 (中等港股标的)
  - 50-54分: B-级 (中下港股标的)
  - 其他等级: C+/C/C- (较差标的), D+/D/D- (高风险标的), F (不合格标的)
